...
export function AtlasSection() {
  ...
  return (<section id="atlas" className="section-shell space-y-6"><div className="flex flex-col gap-4"><div><p className="eyebrow">Atlas</p><h2 className="section-title mt-3">사전식이 아닌 질문형 철학자 아틀라스</h2><p className="section-subtitle">질문 → 판단 기준 → 함정 포인트로 즉시 구분하세요.</p></div>
  ...
  <div className="grid gap-5 sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-3">{list.map((p) => <PhilosopherCard key={p.id} philosopher={p} />)}</div></section>);
}
